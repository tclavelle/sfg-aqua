...
fish2 <- fish %>%
  filter(!country %in% c('Totals - Quantity (tonnes)', 'Totals - Quantity (number)')) %>%
  group_by(country, year) %>%
...
```{r}
ranks_df %>%
  filter(prod_rank <= 20 & country != 'China') %>%
  ggplot(aes(x = country, y = prod_perc, size = five_yr_avg)) +
  geom_point() +
  coord_flip()
```
...
ranks_dense <- ranks_df %>%
  ...
  mutate(net_rank   = prod_rank - fish_prod_rank,
         ...
         catch_to_aqua = catch / aqua_harvest) %>%
  ungroup()
...
ranks_dense$five_yr_avg[ranks_dense$five_yr_avg > 50] <- 50
