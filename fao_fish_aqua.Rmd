...
isscaap_comps <- aqua %>%
  ...
  rename(country      = Country,
         ...
         aqua_harvest = AquaProduction,
         aqua_value   = AquaValue) %>%
  group_by(year, fao_area, comm_name, isscaap) %>%
  summarize(aqua_harvest = sum(aqua_harvest, na.rm = T)) %>%
  ungroup() %>%
    left_join(fish %>%
              select(year, fao_area, comm_name, isscaap, catch) %>%
              group_by(year,  fao_area, comm_name, isscaap) %>%
              summarize(catch = sum(catch,na.rm = T))) %>%
  filter(year == 2013 & is.na(catch)==F)



isscaap_comps %>%
  filter(aqua_harvest > 0 & catch > 0 & !grepl('nei', comm_name)) %>%
  ggplot(aes(x = catch, y = aqua_harvest, fill = isscaap)) +
  geom_point() +
